...
1. **GenAI Coding 模式出現後「先動手 ⭢ 再 refine」變得效率很高**
2. 用「需求描述」的方式就可以寫出程式，激發靈感與創意，成就感高
3. 給模型一句 prompt ⭢ AI 輸出 code ⭢ 測試 ⭢ 調整 prompt ⭢ 迭代優化
4. 比起「先花 X 天寫 PRD」更快看到產品雛形

> **一句話描述**：Vibe Coding 強調的是 coding 過程中的探索與創造，而不是執行任務為導向。
...
1. **容易變成丟棄式程式碼：**
   - 不是先定 spec，缺少邏輯分層
   - 偏向程式草圖，不好拆 component、難 refactor、難擴充
...
| 特性 | **Spec-Driven Development (SDD)** | **Test-Driven Development (TDD)** |
| :--- | :--- | :--- |
| **核心理念** | **規格優先** | **測試優先** |
| **驅動依據** | 一份詳盡的 Spec | 一個會失敗的「單元測試」 |
| **第一步** | 撰寫描述「What」和「Why」的產品規格。 | 撰寫一個小型的、會失敗的測試案例。 |
| **主要產物** | 規格文件| 可運行的測試程式碼 |
| **主要目標** | 確保產品**符合業務/產品意圖**。 | 確保程式碼**功能正確、穩定且易於重構**。 |
| **維度層級** | 較宏觀<br/>關注系統行為、使用者故事 | 較微觀<br/>關注單一函式、類別的功能 |
| **典型流程** | 1. 定義規格<br>2. 規劃技術<br>3. 實作 (常搭配 AI)<br>4. 驗證 | 1. 寫失敗的測試 (紅)<br>2. 寫剛好通過的程式 (綠)<br>3. 重構程式碼 (重構) |
...
- 優點
    ...
    - **AI 輔助釐清**：在 `/speckit.clarify` 過程中，AI 會主動提出不同面向的需求衝突並協助釐清
...
- 價值
    ...
    - **品質控管**：會自動基於 TDD 概念建立單元測試，規定成功率達標才算完成
